...
function CheckoutForm({ selectedPlan, amount }: { selectedPlan: string, amount: number }) {
...
function getPlanDescription(plan: string): string {
  switch (plan) {
    case 'single':
      return '5 inserzioni utilizzabili quando vuoi, validità illimitata';
    case 'standard':
...
export default function Checkout() {
  ...
  const { user } = useAuth();
  const { toast } = useToast();
  // Utilizziamo useRoute per ottenere i parametri dell'URL in modo sicuro
  const [match, params] = useRoute('/checkout/:plan?');
  const planFromQuery = match ? params.plan : null;

  useEffect(() => {
    // Imposta il piano dai parametri dell'URL se presente
    if (planFromQuery && ['single', 'standard', 'premium'].includes(planFromQuery)) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              ...
              <Card className={`cursor-pointer transition-all hover:border-primary ${selectedPlan === 'single' ? 'border-2 border-primary' : ''}`}
                ...
                <CardHeader className="pb-2">
                  <CardTitle className="text-lg">Pacchetto 5 inserzioni</CardTitle>
